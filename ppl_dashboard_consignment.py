...
def process_data(df_sales_raw, df_db_raw, df_dist_raw, df_waste_raw, report_type,df_uom_raw=None,df_dist2_raw=None,df_loc_raw=None):
        ...
        if df_loc is not None:
            ...
            for _, row in df_loc.iterrows():
                ...
                if "AEON" in report_type:
                    ...
                    tc = str(row.get('TfpLoc', '')).replace('.0', '').strip()
    ...
    # Direct system pairing alignment
    df_sales['NAV'] = df_sales['NAV'].apply(clean_id)
    ...
    if 'Store' in df_dist.columns and dist_name_col:
        for _, row in df_dist.dropna(subset=['Store', dist_name_col]).iterrows():
            ...
            # Avoid overwriting with item descriptions by checking if string looks like a store name or code layout
            if raw_code and raw_name and raw_code not in ["0", "NAN", "NONE", "UNKNOWN"] and not raw_code.startswith("Item"):
                # Clean up specific formatting if JG appends code to external doc names
                if '-' in raw_name: raw_name = raw_name.split('-')[-1].strip()
                dynamic_store_lookup[raw_code] = raw_name
    ...
    def process_live_sales_store(x):
        ...
        if code == "" or code == "0" or code.upper() in ["NAN", "NONE"]: 
            return "UNKNOWN"
        return dynamic_store_lookup.get(code, f"Store {code}")

    df_sales['Store'] = df_sales['Store'].apply(process_live_sales_store)
    ...
    if 'Date' in df_sales.columns:
        ...
        elif report_type in ['AEON', 'AEON DF', 'TFP', 'TFP DF','JG','JG DF']:
    ...
    if df_dist2_raw is not None and not df_dist2_raw.empty:
        ...
        if df_dist2 is not None:
            ...
            if report_type in ['AEON', 'AEON DF']:
                ...
                mask = df_dist2['Store'].astype(str).str.upper().str.contains('JG', regex=True, na=False)
                df_dist2 = df_dist2[mask]
            # --------------------------------------------------------------------------
...
def main_app_interface(authenticator, name, permissions):
    st.title("PPL Consignment Report")
    with st.sidebar:
        ...
        with b5:
            if can_view("JG") and st.button("JG Vege"):
                st.session_state['report_type'] = "JG"
                st.session_state['urls'] ={
                    's': make_url(st.secrets["sheet_ids"]["jg_sales"]),
                    'db': make_url(st.secrets["sheet_ids"]["tfp_dry_db"]),
                    'd': make_url(st.secrets["sheet_ids"]["jg_dist"]),
                    'w': make_url(st.secrets["sheet_ids"]["jg_waste"]),
                    'h': make_url(st.secrets["sheet_ids"]["jg_history"])
                }
                st.rerun()
        with b6:
            if can_view("JG") and st.button("JG DF"):
                st.session_state['report_type'] = "JG DF"
                st.session_state['urls']={
                    's': make_url(st.secrets["sheet_ids"]["jg_dry_sales"]),
                    'd': make_url(st.secrets["sheet_ids"]["jg_dry_dist"]),
                    'db': make_url(st.secrets["sheet_ids"]["tfp_dry_db"]),
                    'd2': make_url(st.secrets["sheet_ids"]["aeon_dry_dist_2"]),
                    'w': make_url(st.secrets["sheet_ids"]["jg_dry_waste"]),
                    'h': make_url(st.secrets["sheet_ids"]["jg_dry_history"])
                }
                st.rerun()
        b7,b8 =st.sidebar.columns(2)
        with b7:
            if can_view("GCH") and st.button("GCH Vege"):
                st.session_state['report_type'] = "GCH"
                st.session_state['urls']={
                    's': make_url(st.secrets["sheet_ids"]["gch_sales"]),
                    'db': make_url(st.secrets["sheet_ids"]["tfp_dry_db"]),
                    'd': make_url(st.secrets["sheet_ids"]["gch_dist"]),
                    'w': make_url(st.secrets["sheet_ids"]["gch_waste"]),
                    'h': make_url(st.secrets["sheet_ids"]["gch_history"])
                }
                st.rerun()
        with b8:
            if can_view("GCH") and st.button("GCH DF"):
                st.session_state['report_type'] ="GCH DF"
                st.session_state['urls']={
                    's': make_url(st.secrets["sheet_ids"]["gch_dry_sales"]),
                    'db': make_url(st.secrets["sheet_ids"]["gch_dry_db"]),
                    'd': make_url(st.secrets["sheet_ids"]["gch_dry_dist"]),
                    'w': make_url(st.secrets["sheet_ids"]["gch_dry_waste"]),
                    'h': make_url(st.secrets["sheet_ids"]["gch_dry_history"])
                }
                st.rerun()
        st.markdown("---")
    ...
    if app_mode == "📡 Live Analysis":
        with st.spinner(f"Fetching Live Data for {rpt}..."):
            ...
            r_db = load_google_sheet(urls['db']) if 'db' in urls else None
